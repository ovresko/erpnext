...
def get_items(start, page_length, price_list, item_group, search_value="", pos_profile=None,item_manufacturer=None, vehicule_marque=None, vehicule_modele=None, vehicule_generation=None, vehicule_version=None):
	...
	if display_items_in_stock == 0:
		res = frappe.db.sql("""select i.name as item_code, i.item_name, i.image as item_image, i.idx as idx,
			...
			,i.critere_text  ,MATCH(i.name,i.item_name,i.nom_generique_long,i.manufacturer_part_no) as relevance
			from `tabItem` i 
			LEFT JOIN
			...
			where
				...
				and i.item_group in (select name from `tabItem Group` where lft >= {lft} and rgt <= {rgt})
		        	and {condition} order by relevance desc limit {start}, {page_length}""".format(start=start,page_length=page_length,lft=lft, rgt=rgt,condition=condition),
			{
				'item_code': item_code,
		...
		query = """select i.name as item_code, i.item_name, i.image as item_image, i.idx as idx,
				i.is_stock_item, item_det.price_list_rate, item_det.currency, i.oem_text,i.titre_article,
				i.manufacturer,i.manufacturer_part_no,i.fabricant_logo , i.critere_text 
				,MATCH(i.name,i.item_name,i.nom_generique_long,i.manufacturer_part_no) as relevance
...
def get_conditions(item_code, serial_no, batch_no, barcode):
	...
	condition = """ MATCH AGAINST(%(item_code)s IN NATURAL LANGUAGE MODE)  """
