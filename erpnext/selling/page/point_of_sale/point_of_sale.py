...
def get_items(start, page_length, price_list, item_group, search_value="", pos_profile=None,item_manufacturer=None,item_modele=None, vehicule_marque=None, vehicule_modele=None, vehicule_generation=None, vehicule_version=None):
	...
	if display_items_in_stock == 0:
		res = frappe.db.sql("""select i.name as item_code,i.variant_of,i.has_variants, i.item_name, i.image as item_image, i.idx as idx,i.clean_manufacturer_part_number,
		...
		query = """select i.name as item_code,i.variant_of,i.has_variants, i.item_name, i.image as item_image, i.idx as idx,i.clean_manufacturer_part_number,
				i.is_stock_item, item_det.price_list_rate, item_det.currency, i.oem_text,i.titre_article,i.manufacturer,i.manufacturer_part_no,i.fabricant_logo , i.critere_text  
...
def get_conditions(item_code, serial_no, batch_no, barcode):
	...
	condition = """ (  i.clean_manufacturer_part_number LIKE '%%{}%%' or i.oem_text LIKE '%%{}%%' or MATCH(i.nom_generique_long,i.manufacturer_part_no,i.clean_manufacturer_part_number) AGAINST('({}) ({})' IN NATURAL LANGUAGE MODE)  )""".format(item_code,item_code,keyword,item_code)
...
def get_item_generation(vehicule_generation):
	cond = """ and (i.name in (select parent from `tabVersion vehicule item` where version_vehicule LIKE '{}%%') or i.name in (select parent from `tabGeneration vehicule item` where generation_vehicule = '{}') )""".format(vehicule_generation,vehicule_generation)
	return cond

def get_vehicule_modele(vehicule_modele):
	cond = """ and (i.name in (select parent from `tabVersion vehicule item` where version_vehicule LIKE '{0}%%')  or i.name in (select parent from `tabModele vehicule item` where modele_vehicule = '{}') )""".format(vehicule_modele,vehicule_modele)
	return cond

def get_item_marque(vehicule_marque):
	cond = """ and (i.name in (select parent from `tabVersion vehicule item` where version_vehicule LIKE '{0}%%') or i.name in (select parent from `tabMarque vehicule item` where marque_vehicule = '{}') )""".format(vehicule_marque,vehicule_marque)
	return cond
