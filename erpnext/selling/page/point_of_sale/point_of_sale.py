...
def open_item_info(item_code):
	if item_code:
		...
		_modal =""" 
						...
						<button  class="btn btn-default btn-sm  btn-analyse" data-item-code="{variant_of}"  style="margin-right: 5px;">Analyse</button>
<br>
						<div class="etat-stock"></div>
						<div class="etat-price"></div>
						<div class="etat-val"></div>
						...
						<div>{composant_text}</div>
						<hr>""".format(item_name=item.item_name,image=image,item_code=item_code,manufacturer_part_no=item.manufacturer_part_no
							    ,oem_text=item.oem_text,manufacturer=item.manufacturer,fabricant_logo=fabricant_logo,critere_text=item.critere_text,
							       articles_text=item.articles_text,composant_text=item.composant_text,variant_of= item.variant_of)
...
def get_item_prices(item_code,selling=1):
	...
	if prices:
		for p in prices:
			text += "<li>%s [ Qts > %s ]      <strong style='color: #005fef;'>%s</strong> %s </li>" % ((p.price_list or '').ljust(25,str(" ")),(str(p.min_qty) or '').ljust(20,str(" ")),(str(p.price_list_rate) or '').rjust(20,str("_")),p.currency)
...
def prepare_bulk_print_html(names,customer,warehouse):
	...
	{% for sc in names %}<small>{{sc}} {{names[sc].item}} : <span style="font-weight:bold"">{{names[sc].qts}} <span> ************************ <span>{{names[sc].adr}}<span>
