...
def get_item_prices(item_code,selling=1):
	...
	if prices:
		for p in prices:
			text += "<li>%s		 +%s	 %s %s </li>" % ((p.price_list or '').ljust(20,str("-")),(str(p.min_qty) or '').ljust(5,str(" ")),(str(p.price_list_rate) or ''),p.currency)
...
def get_item_info(item_code,price_list):
	...
	if prices:
		for p in prices:
			text += "<li>%s +%s <strong>%s</strong> %s </li>" % ((p.price_list or '').ljust(20,str(" ")),(str(p.min_qty) or '').ljust(10,str(" ")),(str(p.price_list_rate) or ''),p.currency)
	res.update({'price':text})
	return res
...
def get_items(start, page_length, price_list, item_group, search_value="", pos_profile=None,item_manufacturer=None,item_modele=None, vehicule_marque=None, vehicule_modele=None, vehicule_generation=None, vehicule_version=None,item_oem=None,parent_item_group=None):
	...
	if display_items_in_stock == 0:
		res = frappe.db.sql("""select i.name as item_code,item_adr.adresse,item_adr.warehouse,i.nbr_variante ,i.qts_total,i.designation_commerciale,i.variant_of,i.has_variants, i.item_name, i.image as item_image, i.idx as idx,i.clean_manufacturer_part_number, i.composant_text,i.articles_text,
		...
		query = """select i.name as item_code,i.variant_of,item_adr.adresse,item_adr.warehouse,i.qts_total,i.designation_commerciale,i.nbr_variante ,i.has_variants, i.item_name, i.image as item_image, i.idx as idx,i.clean_manufacturer_part_number,i.composant_text,i.articles_text,
