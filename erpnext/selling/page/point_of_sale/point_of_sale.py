...
def get_items(start, page_length, price_list, item_group, search_value="", pos_profile=None,item_manufacturer=None,item_modele=None, vehicule_marque=None, vehicule_modele=None, vehicule_generation=None, vehicule_version=None):
	...
	if display_items_in_stock == 0:
		res = frappe.db.sql("""select i.name as item_code,i.variant_of,i.has_variants, i.item_name, i.image as item_image, i.idx as idx,i.clean_manufacturer_part_number, i.composant_text,i.articles_text,
			...
			ON
				(item_bin.item_code=i.name or item_bin.item_code=i.variant_of) GROUP BY i.item_code
			where
		...
		query = """select i.name as item_code,i.variant_of,i.has_variants, i.item_name, i.image as item_image, i.idx as idx,i.clean_manufacturer_part_number,i.composant_text,i.articles_text,
...
def get_vehicule_modele(vehicule_modele):
	cond = """ and (i.name in (select parent from `tabVersion vehicule item` where version_vehicule LIKE '{}%%')  or i.name in (select parent from `tabModele vehicule item` where modele_vehicule = '{}') )""".format(vehicule_modele,vehicule_modele)
	return cond

def get_item_marque(vehicule_marque):
	cond = """ and (i.name in (select parent from `tabVersion vehicule item` where version_vehicule LIKE '{}%%') or i.name in (select parent from `tabMarque vehicule item` where marque_vehicule = '{}') )""".format(vehicule_marque,vehicule_marque)
	return cond
