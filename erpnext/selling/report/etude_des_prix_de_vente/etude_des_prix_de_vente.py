...
def execute(filters=None):
	...
	for mri in mitems:
		...
		pond_valuation = 0
		pondere = frappe.db.sql("""select avg(medium) 
		from ( 
		select t.item_code, (sum(t.actual_qty) * sum(t.valuation_rate)) AS medium from `tabStock Ledger Entry` t  
		GROUP BY t.item_code
		where item_code=%s and actual_qty>0
		) as inner_query """, (mri.item_code), as_dict=1)
		if pondere:
