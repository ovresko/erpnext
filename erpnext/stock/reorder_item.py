...
def refresh_refs():
	frappe.db.sql("""
	update tabItem set clean_manufacturer_part_number= REPLACE(REPLACE(REPLACE(REPLACE(manufacturer_part_no,' ',''),'-',''),'.',''),'/','') where (manufacturer_part_no != '' and manufacturer_part_no IS NOT NULL) 
	""")
def refresh_items():
