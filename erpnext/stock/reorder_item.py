...
def refresh_refs():
	...
	""")
	frappe.db.sql("""
	update tabItem as t1 set t1.nbr_variante=(select res.mcount from (select it.nbr_variante as mcount from `tabItem` as it where it.name=t1.variant_of limit 1) as res)  where t1.has_variants=0  
	""")
