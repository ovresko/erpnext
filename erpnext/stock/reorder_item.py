...
def refresh_refs():
	frappe.db.sql("""
	update `tabSupplier Quotation Item` as sqi set sqi.handled=1 
	where exists (select supplier_quotation_item from `tabPurchase Order Item` as poi where poi.docstatus=1 
	...
	""")
	frappe.db.sql("""
	update tabItem as t1 set t1.nbr_variante=(select res.mcount from (select count(it.variant_of) as mcount from `tabItem` as it where it.variant_of=t1.name) as res)  where t1.has_variants=1 
	""")
	frappe.db.sql("""
	update tabItem as i  set i.nbr_variante=( select res.mcount from ( select it.nbr_variante) as mcount from `tabItem` as it where it.name=i.variant_of limit 1)  as res)   where i.has_variants=0  
	""")
...
def get_item_warehouse_projected_qty(items_to_consider):
	...
	for item_code, warehouse, projected_qty in frappe.db.sql("""select item_code, warehouse, projected_qty
		...
		print("%s projected_qty in %s : %d" % (item_code,warehouse,projected_qty))
		while warehouse_doc.parent_warehouse:
			if not item_warehouse_projected_qty.get(item_code, {}).get(warehouse_doc.parent_warehouse):
				print("%s parent_warehouse in %s : %d" % (item_code,warehouse_doc.parent_warehouse,projected_qty))
