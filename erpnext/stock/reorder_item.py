...
def refresh_refs():
	...
	""")
	frappe.db.sql("""
	update tabItem as i set i.nbr_variante=(select res.mcount from (select count(it.variant_of) as mcount from `tabItem` as it where it.variant_of= i.name) as res)  where i.has_variants=1 
	""")
	frappe.db.sql("""
