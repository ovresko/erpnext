...
def execute(filters=None):
	...
	for model in models:
		...
		for mri in mitems:
			...
			relq = frappe.db.sql("""select sum(po.qty) - sum(pi.qty) as 'diffr' from `tabPurchase Invoice Item` pi, `tabPurchase Order Item` po where pi.item_code=%s and po.item_code=%s and pi.docstatus=1 and po.docstatus=1 GROUP BY pi.item_code""", (mri.item_code,mri.item_code))[0]
			if relq:
				relq = relq[0]
			else:
				relq = ''
