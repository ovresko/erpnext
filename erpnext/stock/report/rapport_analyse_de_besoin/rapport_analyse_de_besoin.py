...
def execute(filters=None):
	...
	for model in models:
		...
		for mri in mitems:
			...
			row = ["""<input type='button' href="javascript:open_item_info('%s')" type='b' value='info'>  </input> &nbsp;&nbsp;&nbsp; <button id='%s' onClick="demander_item('%s')" type='button'>Demander</button><input placeholder='Qts' id='input_%s' style='color:black'></input><button   onClick="achat_item('%s')" type='button'>ACHAT %s</button>""" % (mri.item_code,mri.item_code,mri.item_code,mri.item_code,mri.item_code,mri.is_purchase_item),
			       mri.item_code,
			       #date
			       date,
			       mri.item_name,
