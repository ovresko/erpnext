...
def execute(filters=None):
	...
	items = frappe.db.sql(
		...
		where (it.item_code = tmri.item_code or it.variant_of = tmri.model ) and disabled=0 and has_variants=0 {conditions}
		{order_by_statement}
		""".format(
		...
		for mri in mitems:
			...
			if hasattr(mri, 'tconsultation'):
				...
				if mri.titem_code == mri.item_code:
					...
					_bon_commande = frappe.db.sql("""select handled_cmd from 'tabSupplier Quotation Item' where material_request_item = '{torigin}' """.format(torigin=mri.torigin), as_dict=1)
