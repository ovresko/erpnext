...
class MaterialRequest(BuyingController):
	...
	def handle_per_consulted(self):
		...
		if consulted > 0:
			per = float(consulted) / float(item_count)
		result = float(per * 100)
		self.per_consulted = result
		if result > 0:
			self.status = "Consultation"
			frappe.db.sql("""update `tabMaterial Request` set per_consulted = %s,status = %s where name = %s""", (result,"Consultation", self.name))
		else:
			frappe.db.sql("""update `tabMaterial Request` set per_consulted = %s,status = %s where name = %s""", (result,"Non consulte", self.name))
...
def get_mr_items(manufacturer):
	if not manufacturer:
		return []
	_ids = frappe.db.sql("""select item_code, pays, name, model, consultation, consulted, parent, item_name, qty, ref_fabricant, fabricant from `tabMaterial Request Item` where warehouse='GLOBAL - MV' and consulted=0 and docstatus=1 and fabricant=%s """,(manufacturer), as_dict=1)
	if not _ids:
		_ids = []
	return _ids
...
def get_supplier_quotation(manufacturer):
	ids = []
	_ids = frappe.db.sql("""select parent from `tabMaterial Request Item` where consulted=0 and fabricant=%s """,(manufacturer), as_dict=1)
	for i in _ids:
		ids.append(i.parent)
